...
export class ProductionPlansController {
  ...
  @ApiResponse({
    status: 400,
    description: '참조하는 데이터가 존재하지 않음 (제품/규격)',
  })
  @ApiResponse({
    status: 401,
  ...
  @ApiResponse({
    status: 200,
    description: '생산 계획 정보 (제품, 포장 규격 포함)',
  })
  ...
  @Get()
  @ApiOperation({ summary: '생산 계획 목록 조회 (날짜 범위 필터 가능)' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 401, description: '인증 실패' })
  getProductionPlans(@Query() dateRange: DateRagneQueryDto) {
    return this.productionPlansService.getProductionPlans(dateRange);
  }
